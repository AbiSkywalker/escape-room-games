...
$error = 0;
 $mensaje = "";
   ...
   if (isset($_GET["error"])){
           ...
           if ($_GET['error'] == -1)
                    $mensaje = "<span class='text-info'>Accede con un código de partida</span>";
                    
           elseif ($_GET['error'] == -2)
                    $mensaje = "<span class='text-info'>No se encuentra ninguna partida con ese código</span>";
   
   }else{
   
        
           if (isset($_POST["codigo_partida"])){
                 $codigo_partida = $_POST["codigo_partida"];
                     ...
                     $query = $conn->prepare("INSERT INTO panoplia_partida (codigo, pruebas_hechas) VALUES (?, ?)");
