        ...
        $error = 0;
         $mensaje = "";
           ...
           if (isset($_GET["error"])){
                   ...
                   if ($_GET['error'] == -1)
                            $mensaje = "<span class='text-info'>ERROR 1</span>";
                            
                   elseif ($_GET['error'] == -2)
                            $mensaje = "<span class='text-info'>ERROR 2</span>";
                   ...
                   if (isset($_POST["nombrejugador"])){
                         ...
                         //comprobar si existe el nombre de jugador
                         $query = $conn->prepare("SELECT id FROM jugadoresreguntas WHERE nombre = ? and idpartida = ?");
                         $query->bind_param("s", $nombrejugador, $idgrupo);
                         $query->execute();
                         $res = $query->fetch();
                         $query->close();
                         
                         if($res){
                                 $error = -1; //el nombre de jugador ya existe
                                 $mensaje = "<span class='err'>Ya existe un jugador con ese nombre en esta partida, elige otro</span>";
                         }else{
                             
                             
                             //COMPROBAR ADMIN
                             $query = $conn->prepare("COUNT id FROM jugadoresreguntas WHERE idpartida = ?");
                             ...
                             $query = $conn->prepare("INSERT INTO jugadoresreguntas (nombre, isadmin) VALUES (?, ?)");
                             $query->bind_param("ss", $nombrejugador, $isadmin);
                             $query->execute();
